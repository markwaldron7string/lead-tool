...
const HARD_EXCLUDE_KW = [
  ...
  "mortgage", "home loan", "home loans", "personal loan", "personal loans",
...
function classify(row, sourceName) {
  ...
  // Exclude if strong bad signal with no good signal
  if (hasBad && !hasGood) return "EXCLUDED";

  // Also exclude if Google category is clearly not a buyers agent
  ...
  if (BAD_CATEGORIES.some(c => categoryText.includes(c)) && !hasGood) {
    return "EXCLUDED";
  }
