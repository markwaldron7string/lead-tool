...
function classify(row, sourceName) {
  ...
  const catFields = Array.from(
    { length: 10 },
    (_, i) => row[`categories/${i}`] || "",
  );
  const text = [
    row.title || "",
    row.categoryName || "",
  ...
  if (
    row._category &&
    row._category !== "Uncategorised" &&
    row._category !== "EXCLUDED"
  ) {
...
export function processFiles(fileDataArray) {
  ...
  for (const file of fileDataArray) {
    ...
    for (const row of file.rows) {
      ...
      const emails = row.emails || row.Email || row.Emails || "";
      const founderName = row.founder_name || row["Founder Name"] || "";

      if (seen.has(key)) {
        // Don't discard — merge email/founder if the existing record is missing them
        const existing = seen.get(key);
        if (!existing.emails && emails) existing.emails = emails;
        if (!existing.founder_name && founderName)
          existing.founder_name = founderName;
        dupes++;
        continue;
      }
  ...
  const hasName = leads.filter(
    (r) => r.founder_name && r.founder_name.trim(),
  ).length;
  ...
  for (const lead of leads) {
    ...
    categoryBreakdown[lead._category] =
      (categoryBreakdown[lead._category] || 0) + 1;
  }
...
export function filterLeads(leads, { search, state, category }) {
  return leads.filter((lead) => {
    if (search) {
      ...
      const haystack =
        `${lead.title} ${lead.city} ${lead.phone} ${lead.website} ${lead.emails}`.toLowerCase();
