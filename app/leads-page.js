        ...
        >
          <div style={{ width: "100%", maxWidth: 700 }}>
            ...
            <div style={{ marginBottom: 28 }}>
              <Link
                ...
                style={{
                  ...
                  padding: "6px 12px", transition: "border-color 0.15s, color 0.15s",
                }}
            ...
            <div style={{ textAlign: "center", marginBottom: 36 }}>
              ...
              <h1 style={{ fontSize: isMobile ? 20 : 26, fontWeight: 600, letterSpacing: "-0.02em", marginBottom: 12, color: "var(--text)" }}>
                {title}
              </h1>
              <p style={{ fontSize: 15, color: "var(--muted)", margin: 0 }}>
                No leads yet. Start by scraping Google Maps.
              </p>
            </div>

            {/* Scrape panel — open by default */}
            <ScrapePanel
              ...
              <div className="enrich-bar">
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "var(--muted)" }}>
                    Add leads first to enable enrichment
                  </div>
                </div>
                <button
                  disabled
                  style={{ background: "var(--green)", color: "#0a0a0b", fontWeight: 600, fontSize: 13, padding: "8px 16px", borderRadius: 7, display: "flex", alignItems: "center", gap: 7 }}
                >
                  ✦ Enrich all leads
                </button>
              </div>
            ...
            {/* Drop zone for uploading existing CSVs */}
            <div
      ...
      <div style={{ maxWidth: 1600, margin: "0 auto", padding: isMobile ? "0 16px 60px" : "0 24px 80px" }}>
        ...
        <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: isMobile ? "20px 0 16px" : "28px 0 24px", borderBottom: "1px solid var(--border)", marginBottom: isMobile ? 16 : 28, gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            >←</Link>
            <div>
              <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--green)", letterSpacing: "0.06em", marginBottom: 2, cursor: "default" }}>LEAD SCRAPER</div>
              <h1 style={{ fontSize: isMobile ? 16 : 20, fontWeight: 600, letterSpacing: "-0.02em", lineHeight: 1, cursor: "default" }}>{title}</h1>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexShrink: 0 }}>
            ...
            <button
              ...
              style={{ background: filtered.length ? "var(--green)" : "var(--surface2)", color: filtered.length ? "#0a0a0b" : "var(--muted)", fontWeight: 600, fontSize: isMobile ? 12 : 13, padding: isMobile ? "7px 12px" : "8px 18px", borderRadius: 8, opacity: filtered.length ? 1 : 0.4, cursor: filtered.length ? "pointer" : "not-allowed" }}
            >
        ...
        <div className="enrich-bar">
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            {bulkRunning ? (
              <button onClick={cancelEnrich} style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--red)", borderRadius: 7, padding: "7px 14px", fontSize: 13 }}>
                Cancel
              </button>
            ) : (
              <button onClick={enrichAll} disabled={!leads.length} style={{ background: "var(--green)", color: "#0a0a0b", fontWeight: 600, fontSize: 13, padding: "8px 16px", borderRadius: 7, display: "flex", alignItems: "center", gap: 7 }}>
                ✦ Enrich all leads
              </button>
            )}
          </div>
        ...
        <div className="toolbar">
          ...
          {hasActiveFilters && (
            <button
              onClick={() => { setSearch(""); setFilterState(""); setFilterCategory(""); setFilterScore(0); setPage(1); }}
          ...
          <button
            onClick={() => fileInputRef.current?.click()}
            style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--text)", borderRadius: 6, padding: "9px 14px", fontSize: 13, display: "flex", alignItems: "center", gap: 6, whiteSpace: "nowrap", flexShrink: 0 }}
          >
        ...
        <div style={{ marginBottom: 12 }}>
          <button
            ...
            style={{ background: "none", border: "none", color: showAdvanced ? "var(--text)" : "var(--muted)", fontSize: 12, cursor: "pointer", padding: "4px 0", display: "flex", alignItems: "center", gap: 6, transition: "color 0.15s" }}
          >
            <span style={{ fontSize: 10 }}>{showAdvanced ? "▼" : "▶"}</span>
            Advanced filters
            {filterSource && (
              <span style={{ background: "rgba(62,207,142,0.15)", color: "var(--green)", borderRadius: 99, padding: "1px 8px", fontSize: 11, fontFamily: "var(--font-mono)" }}>1 active</span>
            )}
          </button>
          {showAdvanced && (
        ...
        {loadedFiles.length > 1 && (
          <div className="file-summary">
            <div className="file-summary-stats">
              ...
              <span style={{ color: "var(--muted)" }}>
                <span style={{ fontFamily: "var(--font-mono)", color: "var(--green)", marginRight: 5 }}>{stats?.unique?.toLocaleString()}</span>unique leads
              </span>
                ...
                {visibleCols.map((col) => (
                  <th key={col.key} onClick={() => handleSort(col.key)} style={{ cursor: "pointer", userSelect: "none", position: "relative", overflow: "visible" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                      ...
                      {col.key === "_score" && (
                        <ScoreInfoTooltip onFilter={(score) => { setFilterScore(score); setPage(1); }} />
                      )}
                    </div>
