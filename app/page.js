...
const DEFAULT_COLS = [
  ...
  { key: 'abn',              label: 'ABN',             width: 130, visible: false },
...
const PRESET_TERMS = [
  ...
  'investment buyers agent', 'SMSF buyers agent', 'property advisor',
  'property acquisition', 'property strategist',
...
function classifyClient(lead) {
  ...
  if (text.includes('off the plan') || text.includes('off-the-plan'))      return 'Off-the-plan';
  if (text.includes('project sales') || text.includes('project marketing')) return 'Project sales';
  if (text.includes('investment') || text.includes('investor'))             return 'Investment BA';
  if (text.includes('first home') || text.includes('owner occupier'))       return 'Owner-occupier';
  if (text.includes('advisor') || text.includes('strategist'))              return 'Property advisor';
...
function ScoreInfoTooltip({ onFilter }) {
  ...
  const rows = [
    ...
    { label: 'Google rating',    pts: 'up to 15', color: '#e8a045', sub: [
      ['4.8+','15 pts'],['4.5–4.7','12 pts'],['4.0–4.4','8 pts'],['3.5–3.9','4 pts'],
    ]},
    { label: 'Review count',     pts: 'up to 10', color: '#e8a045', sub: [
      ['50+ reviews','10 pts'],['20–49','7 pts'],['10–19','4 pts'],['1–9','2 pts'],
    ]},
  ...
  const tooltip = visible && mounted ? createPortal(
    <div ref={tooltipRef} style={{
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom: pinned?12:4 }}>
        <div>
          <div style={{ fontSize:13, fontWeight:500, color:'#efefef' }}>How scores are calculated</div>
          <div style={{ fontSize:11, color:'#666670', marginTop:3 }}>Each lead scored 0–100 on data quality</div>
        </div>
        <button onClick={onClose} style={{ background:'none', border:'none', color:'#666670', cursor:'pointer', fontSize:14, lineHeight:1, padding:'2px 4px' }}
          onMouseOver={e=>e.currentTarget.style.color='#efefef'}
          onMouseOut={e=>e.currentTarget.style.color='#666670'}>✕</button>
      </div>
      {!pinned && (
        <div style={{ fontSize:10, color:'#3ecf8e', marginBottom:10, display:'flex', alignItems:'center', gap:4, opacity:0.8 }}>
          <span style={{ fontSize:9 }}>●</span> Click ⓘ to keep this open
        </div>
      )}
      {rows.map(({ label, pts, color, sub }) => (
        <div key={label}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'5px 0', borderBottom:'0.5px solid rgba(255,255,255,0.07)', fontSize:12 }}>
            <span style={{ display:'flex', alignItems:'center', gap:7, color:'#efefef' }}>
              <span style={{ width:7, height:7, borderRadius:'50%', background:color, flexShrink:0 }}/>
              {label}
            </span>
            <span style={{ fontFamily:'monospace', fontSize:11, fontWeight:600, color, background:'rgba(255,255,255,0.06)', borderRadius:4, padding:'2px 7px' }}>{pts}</span>
          </div>
          {sub?.map(([desc,val]) => (
            <div key={desc} style={{ display:'flex', justifyContent:'space-between', padding:'2px 0 2px 22px', fontSize:11, color:'#666670' }}>
      ...
      <div style={{ marginTop:12, paddingTop:12, borderTop:'0.5px solid rgba(255,255,255,0.07)' }}>
        <div style={{ fontSize:10, color:'#666670', marginBottom:8, textTransform:'uppercase', letterSpacing:'0.06em' }}>Filter by tier</div>
        <div style={{ display:'flex', gap:6 }}>
          {tiers.map(({ value, label, color }) => (
            <button key={value} onClick={e=>handleTierClick(e,value)}
              style={{ flex:1, textAlign:'center', padding:'8px 4px', border:'0.5px solid rgba(255,255,255,0.12)', borderRadius:6, background:'transparent', cursor:'pointer', transition:'all 0.15s' }}
              onMouseOver={e=>{ e.currentTarget.style.background='rgba(255,255,255,0.06)'; e.currentTarget.style.borderColor=color; }}
              ...
              <div style={{ fontSize:10, color:'#666670' }}>{label}</div>
            </button>
          ))}
        </div>
      </div>
    ...
    <>
      <button ref={btnRef} onMouseEnter={onEnter} onMouseLeave={onLeave} onClick={onClick}
        ...
        style={{
          ...
          transition:'all 0.15s', flexShrink:0, padding:0, fontFamily:'var(--font-sans)',
        }}>i</button>
      {tooltip}
    </>
  );
...
function ScrapePanel({ onLeadsFound }) {
  ...
  async function handleScrape() {
    setScraping(true); setResult(null); setError('');
    try {
      ...
      if (!res.ok || data.error) { setError(data.error || 'Scrape failed'); setScraping(false); return; }
      const classified = (data.leads||[]).map(lead => ({ ...lead, _category: classifyClient(lead) }));
      ...
      <div onClick={()=>setOpen(o=>!o)} style={{
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div style={{
            ...
            boxShadow: scraping?'0 0 6px var(--amber)':'0 0 6px var(--green)',
          }}/>
          <div>
            <span style={{ fontSize:13, fontWeight:500 }}>Scrape new leads</span>
            <span style={{ fontFamily:'var(--font-mono)', fontSize:11, color:'var(--muted)', marginLeft:10 }}>Google Places API</span>
          </div>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          {result && <span style={{ fontSize:12, color:'var(--green)', fontFamily:'var(--font-mono)' }}>+{result.added} leads added</span>}
        ...
        <div style={{ padding:'16px 18px' }}>
          <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:12 }}>
            ...
            <div style={{ flex:'1 1 200px' }}>
              <div style={{ fontSize:11, color:'var(--muted)', marginBottom:5, textTransform:'uppercase', letterSpacing:'0.05em' }}>Search term</div>
              <select
                value={PRESET_TERMS.includes(searchTerm)?searchTerm:'__custom__'}
                onChange={e=>{ if(e.target.value!=='__custom__') setSearchTerm(e.target.value); }}
              ...
              <input type="text" value={searchTerm} onChange={e=>setSearchTerm(e.target.value)}
                placeholder="Custom search term" style={{ width:'100%' }}/>
            </div>

            <div style={{ flex:'0 1 160px' }}>
              <div style={{ fontSize:11, color:'var(--muted)', marginBottom:5, textTransform:'uppercase', letterSpacing:'0.05em' }}>City</div>
            ...
            <div style={{ flex:'0 1 120px' }}>
              <div style={{ fontSize:11, color:'var(--muted)', marginBottom:5, textTransform:'uppercase', letterSpacing:'0.05em' }}>Max results</div>
              <select value={maxResults} onChange={e=>setMaxResults(Number(e.target.value))} style={{ width:'100%' }}>
            ...
            <div style={{ flex:'0 0 auto', display:'flex', alignItems:'flex-end' }}>
              <button onClick={handleScrape} disabled={scraping||!searchTerm.trim()} style={{
                background: scraping?'var(--surface2)':'var(--green)',
                color: scraping?'var(--muted)':'#0a0a0b',
                fontWeight:600, fontSize:13, padding:'9px 20px', borderRadius:7,
          ...
          <div style={{ fontSize:12, color:'var(--muted)', marginBottom:8 }}>
            Run the same search term across multiple cities for full national coverage. Duplicates are removed automatically.
          </div>

          {error && (
            <div style={{ background:'rgba(224,82,82,0.1)', border:'1px solid rgba(224,82,82,0.3)', borderRadius:7, padding:'10px 14px', fontSize:13, color:'var(--red)', marginTop:8 }}>
          ...
          {result && (
            <div style={{ background:'rgba(62,207,142,0.08)', border:'1px solid rgba(62,207,142,0.2)', borderRadius:7, padding:'10px 14px', fontSize:13, marginTop:8, display:'flex', gap:20, flexWrap:'wrap' }}>
              <span style={{ color:'var(--green)' }}><strong style={{ fontFamily:'var(--font-mono)' }}>+{result.added}</strong> new leads added</span>
              <span style={{ color:'var(--muted)' }}><strong style={{ fontFamily:'var(--font-mono)' }}>{result.duplicates}</strong> duplicates skipped</span>
              <span style={{ color:'var(--muted)' }}><strong style={{ fontFamily:'var(--font-mono)' }}>{result.found}</strong> found by Google</span>
            </div>
          )}
        </div>
      )}
...
function StatCard({ label, value, color }) {
  const c = { green:'var(--green)', amber:'var(--amber)', red:'var(--red)', blue:'var(--blue)', purple:'var(--purple)' };
  return (
...
function SocialLink({ url, type }) {
  ...
  let label = url;
  try { const u=new URL(url); const p=u.pathname.split('/').filter(Boolean); label=p[p.length-1]||u.hostname; } catch {}
  const colors = { linkedin:'#0a66c2', instagram:'#e1306c', facebook:'#1877f2' };
  return <a href={url} target="_blank" rel="noopener noreferrer" style={{ color:colors[type]||'var(--blue)', textDecoration:'none', fontSize:12 }} title={url}>{label.slice(0,22)}{label.length>22?'…':''}</a>;
}
...
export default function Home() {
  ...
  useEffect(() => {
    function h(e) { if (colPanelRef.current && !colPanelRef.current.contains(e.target)) setShowColPanel(false); }
  ...
  const startResize = useCallback((e, key, currentWidth) => {
    ...
    function onUp() { dragRef.current.active=false; document.removeEventListener('mousemove',onMove); document.removeEventListener('mouseup',onUp); }
    ...
    async function load() {
      try {
        ...
        Papa.parse(text, {
          ...
          complete: ({ data }) => {
            const file = { name:'leads.csv', rows:data };
            const { leads:l, stats:s } = processFiles([file]);
            setLeads(l); setStats(s); setLoadedFiles([file]);
            setIsAutoLoading(false);
          },
          error: () => setIsAutoLoading(false),
        });
      } catch { setIsAutoLoading(false); }
    }
    load();
  }, []);
  ...
  const handleFiles = useCallback((newFiles) => {
    ...
    const parsed = []; let rem = csvFiles.length;
      ...
      Papa.parse(file, {
        ...
        complete: ({ data }) => {
          ...
          if (--rem === 0) {
            setLoadedFiles(prev => {
              const combined = [...prev];
              parsed.forEach(p => { const idx=combined.findIndex(f=>f.name===p.name); if(idx>=0)combined[idx]=p; else combined.push(p); });
              const { leads:l, stats:s } = processFiles(combined);
              setLeads(l); setStats(s); setPage(1); setIsProcessing(false);
              return combined;
            });
          }
        },
  ...
  const handleNewLeads = useCallback((newLeads) => {
    ...
    setLeads(prev => {
      ...
      for (const lead of newLeads) {
        ...
        else { seen.add(key); lead._score = scoreLead(lead); toAdd.push(lead); added++; }
      }
      return [...prev, ...toAdd];
    });
    setPage(1);
  ...
  async function enrichLead(lead) {
    const [aiData, abnData] = await Promise.all([
      lead.website
        ? fetch('/api/enrich', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ website:lead.website, businessName:lead.title, existingEmail:lead.emails||'' }) }).then(r=>r.ok?r.json():{}).catch(()=>({}))
        : Promise.resolve({}),
      fetch('/api/abn', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ businessName:lead.title }) }).then(r=>r.ok?r.json():{}).catch(()=>({})),
    ]);
    return {
  ...
  const enrichOne = useCallback(async (lead) => {
    ...
    try {
      ...
      setLeads(prev => prev.map(l => { if(l.title!==key)return l; const u={...l,...result}; u._score=scoreLead(u); return u; }));
    } catch {}
  ...
  const enrichAll = useCallback(async () => {
    ...
    cancelRef.current=false; setBulkRunning(true);
    setEnrichProgress({ done:0, total:toEnrich.length });
    let done=0;
    for (let i=0; i<toEnrich.length; i+=BATCH_SIZE) {
      if (cancelRef.current) break;
      await Promise.all(toEnrich.slice(i,i+BATCH_SIZE).map(async lead => {
        ...
        setEnriching(prev=>{const n={...prev};delete n[key];return n;});
        done++; setEnrichProgress({done,total:toEnrich.length});
      }));
      if (i+BATCH_SIZE<toEnrich.length) await sleep(BATCH_DELAY);
    }
    setBulkRunning(false); setEnrichProgress(null); cancelRef.current=false;
  }, [leads]);

  const cancelEnrich = () => { cancelRef.current=true; setBulkRunning(false); setEnrichProgress(null); setEnriching({}); };
  ...
  if (sortCol) {
    filtered = [...filtered].sort((a,b) => {
      ...
      if (!isNaN(Number(av))&&!isNaN(Number(bv))&&av!==''&&bv!=='') return (Number(av)-Number(bv))*sortDir;
  ...
  const handleSort = key => { if(sortCol===key)setSortDir(d=>d*-1); else{setSortCol(key);setSortDir(key==='_score'?-1:1);}  setPage(1); };
  const hasActiveFilters = search||filterState||filterCategory||filterScore>0;

  const handleExport = () => {
    const csv=leadsToCSV(filtered);
    const a=Object.assign(document.createElement('a'),{ href:URL.createObjectURL(new Blob([csv],{type:'text/csv'})), download:`buyers_agents_${new Date().toISOString().slice(0,10)}.csv` });
    a.click(); URL.revokeObjectURL(a.href);
  };

  const visibleCols = isMobile
    ? [{key:'title',label:'Business',width:160},{key:'state',label:'State',width:80},{key:'_category',label:'Category',width:130},{key:'emails',label:'Email',width:180}]
  ...
  return (
    <>
      ...
      <div style={{ maxWidth:1600, margin:'0 auto', padding:isMobile?'0 16px 60px':'0 24px 80px' }}>
        ...
        {/* ── Header ── */}
        <header style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:isMobile?'20px 0 16px':'28px 0 24px', borderBottom:'1px solid var(--border)', marginBottom:isMobile?16:28, gap:12 }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <div style={{ width:28,height:28,borderRadius:7,background:'var(--green)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0 }}>
            ...
            <div>
              ...
              <h1 style={{ fontSize:isMobile?16:20,fontWeight:600,letterSpacing:'-0.02em',lineHeight:1 }}>Buyers Agent Australia</h1>
            </div>
          </div>
          <div style={{ display:'flex', gap:8, alignItems:'center', flexShrink:0 }}>
            ...
            <button onClick={handleExport} disabled={!filtered.length} style={{
              ...
              borderRadius:8,opacity:filtered.length?1:0.4,
        ...
        {stats && (
          <div style={{ display:'grid', gridTemplateColumns:isMobile?'repeat(2,1fr)':'repeat(4,1fr)', gap:10, marginBottom:24 }}>
            ...
            <StatCard label="With ABN"      value={hasABN}            color="purple" />
        ...
        {/* ── Scrape panel ── */}
        <ScrapePanel onLeadsFound={handleNewLeads} />
          ...
          <div className="enrich-bar">
            <div>
              <div style={{ fontSize:13, fontWeight:500, marginBottom:3 }}>
                AI Enrichment + ABN Lookup
                <span style={{ fontFamily:'var(--font-mono)',fontSize:11,color:'var(--muted)',marginLeft:10 }}>gpt-4o-mini + ABR</span>
              </div>
              <div style={{ fontSize:12,color:'var(--muted)' }}>
                {enrichedCount>0
                  ? `${enrichedCount.toLocaleString()} leads enriched — contact, LinkedIn, Instagram, ABN`
                  : 'Extracts contact info, LinkedIn, Instagram, and ABN for each lead'}
              </div>
            </div>
            <div style={{ display:'flex',gap:8,alignItems:'center',flexWrap:'wrap' }}>
              {enrichProgress && (
                <div style={{ display:'flex',alignItems:'center',gap:10 }}>
                  ...
                  <span style={{ fontFamily:'var(--font-mono)',fontSize:12,color:'var(--muted)',whiteSpace:'nowrap' }}>{enrichProgress.done} / {enrichProgress.total}</span>
                </div>
              )}
              {bulkRunning
                ...
                : <button onClick={enrichAll} disabled={!leads.length} style={{ background:'var(--green)',color:'#0a0a0b',fontWeight:600,fontSize:13,padding:'8px 16px',borderRadius:7,display:'flex',alignItems:'center',gap:7 }}>✦ Enrich all leads</button>
              }
            </div>
          </div>
        )}
        ...
        <div className="toolbar">
          ...
          />
          <select value={filterScore} onChange={e=>{setFilterScore(Number(e.target.value));setPage(1);}}
          ...
          <select value={filterState} onChange={e=>{setFilterState(e.target.value);setPage(1);}} disabled={!leads.length} style={{ width:'auto',flex:'0 1 auto' }}>
            <option value="">All states</option>
            {stats?.states?.map(s=><option key={s} value={s}>{s}</option>)}
          </select>
          <select value={filterCategory} onChange={e=>{setFilterCategory(e.target.value);setPage(1);}} disabled={!leads.length} style={{ width:'auto',flex:'0 1 auto' }}>
            <option value="">All categories</option>
            {ALL_CATEGORIES.map(c=><option key={c} value={c}>{c}</option>)}
          </select>
          {leads.length>0 && (
            <button onClick={()=>{setHideExcluded(h=>!h);setPage(1);}} style={{
              ...
              borderRadius:6,padding:'9px 14px',fontSize:12,cursor:'pointer',whiteSpace:'nowrap',flexShrink:0,
            }}>{hideExcluded?'✓ Hide excluded':'Show excluded'}</button>
          )}
          {hasActiveFilters && (
            <button onClick={()=>{setSearch('');setFilterState('');setFilterCategory('');setFilterScore(0);setPage(1);}}
              style={{ background:'var(--surface2)',border:'1px solid var(--border)',color:'var(--muted)',borderRadius:6,padding:'9px 14px',fontSize:13,flexShrink:0 }}>Clear</button>
          )}
          {!isMobile && <div className="toolbar-divider"/>}
          {!isMobile && leads.length>0 && (
            <div style={{ position:'relative',flexShrink:0 }} ref={colPanelRef}>
              <button onClick={()=>setShowColPanel(p=>!p)} style={{
                  ...
                  {cols.map(col=>(
                    <div key={col.key} className="col-row" onClick={()=>toggleCol(col.key)}>
                      ...
                      {['linkedin_company','linkedin_personal','instagram','abn','entity_type'].includes(col.key) && (
                        <span style={{ marginLeft:'auto',fontSize:10,color:'var(--purple)',background:'rgba(167,139,250,0.1)',borderRadius:4,padding:'1px 5px' }}>new</span>
                      )}
                    </div>
                  ))}
          ...
          }}>↑ {isMobile?'Add CSVs':'Add more CSVs'}</button>
        ...
        {loadedFiles.length>1 && (
          <div className="file-summary">
            <div className="file-summary-stats">
              <span style={{ color:'var(--muted)' }}><span style={{ fontFamily:'var(--font-mono)',color:'var(--text)',marginRight:5 }}>{loadedFiles.length}</span>files</span>
              <span style={{ color:'var(--muted)' }}><span style={{ fontFamily:'var(--font-mono)',color:'var(--green)',marginRight:5 }}>{stats?.unique?.toLocaleString()}</span>unique leads</span>
            ...
            <button onClick={clearAll} style={{ background:'none',border:'none',color:'var(--muted)',fontSize:12,cursor:'pointer' }}
              ...
              onMouseOut={e=>e.target.style.color='var(--muted)'}>Clear all</button>
          </div>
        )}
        ...
        {loadedFiles.length===0 && !isAutoLoading && (
          <div className={`drop-zone ${isDragging?'dragging':''}`}
            ...
            <div style={{ fontSize:15,fontWeight:500,marginBottom:6 }}>Drag & drop your CSV exports here</div>
            <div style={{ fontSize:13,color:'var(--muted)' }}>All files merged and deduplicated automatically</div>
          </div>
        )}
        ...
        {leads.length>0 && (
          <>
            <div style={{ border:'1px solid var(--border)',borderRadius:10,overflow:'hidden',overflowX:'auto',WebkitOverflowScrolling:'touch' }}>
              <table style={{ tableLayout:'fixed', minWidth:isMobile?400:visibleCols.reduce((s,c)=>s+c.width,0)+60 }}>
                <colgroup>
                  {visibleCols.map(col=><col key={col.key} style={{ width:col.width }}/>)}
                  {!isMobile && <col style={{ width:52 }}/>}
                </colgroup>
                <thead>
                  <tr>
                    {visibleCols.map(col=>(
                      <th key={col.key} onClick={()=>handleSort(col.key)} style={{ cursor:'pointer',userSelect:'none',position:'relative',overflow:'visible' }}>
                        <div style={{ display:'flex',alignItems:'center',gap:5 }}>
                          {col.label}
                          {sortCol===col.key
                            ? <span style={{ opacity:0.7 }}>{sortDir===1?'↑':'↓'}</span>
                            : <span style={{ opacity:0.2 }}>↕</span>
                          }
                          {col.key==='_score' && <ScoreInfoTooltip onFilter={score=>{setFilterScore(score);setPage(1);}}/>}
                        </div>
                        {!isMobile && <div className="resize-handle" onMouseDown={e=>startResize(e,col.key,col.width)} onClick={e=>e.stopPropagation()}/>}
                      </th>
                    ))}
                    {!isMobile && <th style={{ overflow:'visible' }}>Enrich</th>}
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <tr key={i} className={isExcluded?'excluded':''}>
                        {visibleCols.map(({key})=>{
                          ...
                          if (key==='title')             return <td key={key} title={lead.title} style={{ fontWeight:500 }}>{lead.title}</td>;
                          if (key==='_category')         return <td key={key}><Badge category={lead._category}/></td>;
                          if (key==='website')           return <td key={key}>{lead.website?<a href={lead.website.startsWith('http')?lead.website:`https://${lead.website}`} target="_blank" rel="noopener noreferrer" style={{ color:'var(--blue)',textDecoration:'none',fontSize:12 }}>{lead.website.replace(/^https?:\/\/(www\.)?/,'').split('/')[0]}</a>:'—'}</td>;
                          if (key==='emails')            return <td key={key} style={{ fontFamily:'var(--font-mono)',fontSize:11 }}>{lead.emails?<span style={{ color:'var(--green)' }}>{lead.emails.split(',')[0].trim()}</span>:<span style={{ color:'var(--muted)' }}>—</span>}</td>;
                        ...
                        {!isMobile && (
                          <td>{!isExcluded && (
                            <button onClick={()=>enrichOne(lead)} disabled={isEnriching}
                              title="Enrich with AI + ABN"
                              style={{ background:'none',border:'1px solid var(--border)',color:isEnriching?'var(--green)':'var(--muted)',borderRadius:5,padding:'4px 8px',fontSize:11,cursor:'pointer',display:'flex',alignItems:'center',gap:4,transition:'all 0.15s' }}
                              onMouseOver={e=>{if(!isEnriching)e.currentTarget.style.borderColor='var(--green)';}}
            ...
            <div className="pagination">
              ...
              <div style={{ display:'flex',gap:6 }}>
                <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={safePage===1}
                  style={{ background:'var(--surface2)',border:'1px solid var(--border)',color:safePage===1?'var(--muted)':'var(--text)',borderRadius:6,padding:'6px 14px',fontSize:12,opacity:safePage===1?0.4:1,cursor:safePage===1?'not-allowed':'pointer' }}>← Prev</button>
                <button onClick={()=>setPage(p=>Math.min(totalPages,p+1))} disabled={safePage===totalPages}
                  style={{ background:'var(--surface2)',border:'1px solid var(--border)',color:safePage===totalPages?'var(--muted)':'var(--text)',borderRadius:6,padding:'6px 14px',fontSize:12,opacity:safePage===totalPages?0.4:1,cursor:safePage===totalPages?'not-allowed':'pointer' }}>Next →</button>
              </div>
            </div>
          </>
        )}
